...
class BbcWebTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            BbcWebTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = BbcWebTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = BbcWebTestRunner.getenv("PROJECTNAME_TEST_LIVE")
        override = BbcWebTestRunner.getenv("PROJECTNAME_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = BbcWebTestRunner.getenv(key)
        ...
        explain = BbcWebTestRunner.getenv("PROJECTNAME_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if BbcWebTestRunner._test_control is not None:
            return BbcWebTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                BbcWebTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            BbcWebTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = BbcWebTestRunner.load_test_control()
...
def load_env_local():
    BbcWebTestRunner.load_env_local()
...
def entity_list_to_data(lst):
    return BbcWebTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return BbcWebTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return BbcWebTestRunner.load_test_control()


def live_delay_ms():
    return BbcWebTestRunner.live_delay_ms()
