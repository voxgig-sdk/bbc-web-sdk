...
boru-driven command-line client **and** interactive REPL for the BbcWeb
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/bbc-web-cli)
make build
...
./bbc-web-cli load 1 content            # {id:1} shorthand
./bbc-web-cli load '{id:1}' content       # explicit match map

# 5. Override the API base URL for a single call
BBC_WEB_BASE=https://api.example.com ./bbc-web-cli load 1 content

# 6. No arguments -> interactive REPL
./bbc-web-cli
bbc-web> load 1 content
bbc-web> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/bbc-web-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/bbc-web-cli load 1 content
   ```
...
```sh
./bbc-web-cli load 1 content          # scalar shorthand for {id:1}
./bbc-web-cli load '{id:1}' content     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `bbc-web>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/bbc-web-cli`. |
...
(`./bbc-web-cli <expr>`) and interactively (the REPL), and expressions compose the
