// Command BbcWeb-cli is a CLI + REPL for the BbcWeb SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      BBC_WEB_APIKEY (api key), BBC_WEB_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: BBC_WEB_APIKEY carries the API key and
	// BBC_WEB_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("BBC_WEB_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("BBC_WEB_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewBbcWebSDK(opts)
