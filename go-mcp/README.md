...
[MCP](https://modelcontextprotocol.io) server exposing the BbcWeb SDK as
two agent tools — `bbc-web_list` and `bbc-web_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/bbc-web-mcp)
make build

# 2. Provide credentials via the environment
export BBC_WEB_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user bbc-web \
  -- /absolute/path/to/bbc-web-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./bbc-web-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// bbc-web_list: first page of records
{ "entity": "content" }
{ "entity": "content", "query": { } }

// bbc-web_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/bbc-web-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user bbc-web \
     -- "$PWD"/dist/*/bbc-web-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `bbc-web_list` and `bbc-web_load` tools now appear
   in new sessions. Ask the agent to *"list content using bbc-web"*
   and it calls `bbc-web_list` with `{"entity":"content"}`.
...
```sh
export BBC_WEB_APIKEY=sk_live_xxx            # API key
export BBC_WEB_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `bbc-web_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `bbc-web_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `BBC_WEB_APIKEY` | API key sent with every request. |
| `BBC_WEB_BASE` | Optional override of the API base URL. |
...
```sh
./bbc-web-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"bbc-web_load","arguments":{"entity":"content","query":{"id":1}}}}'
```
