...
class BbcWebNetsimFeature extends BbcWebBaseFeature
{
    ...
    private function _simulate(BbcWebContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(BbcWebContext $ctx, array $applied): void
    {
        $client = $this->client;
