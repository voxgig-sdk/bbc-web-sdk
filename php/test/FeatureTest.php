...
class FtCtrl extends BbcWebControl
{
...
class FtClient
{
    ...
    private BbcWebUtility $utility;

    public function __construct(BbcWebUtility $utility, array $options)
    {
    ...
    public function get_utility(): BbcWebUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (BbcWebContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new BbcWebUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (BbcWebContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = BbcWebConfig::make_feature($name);
    ...
    private static function buildUrl(BbcWebSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof BbcWebError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new BbcWebResponse($fetched);
        }
    ...
    private function fail(BbcWebContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (BbcWebContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
