...
require_once __DIR__ . '/../bbcweb_sdk.php';
...
{
    private static function utility(): BbcWebUtility
    {
        return new BbcWebUtility();
    }
    ...
    private static function ctx(array $over = []): BbcWebContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new BbcWebContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new BbcWebOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof BbcWebError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new BbcWebOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = BbcWebMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new BbcWebOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = BbcWebMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new BbcWebOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = BbcWebMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new BbcWebSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = BbcWebMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new BbcWebResponse([]),
            'result' => new BbcWebResult([]),
        ]);
        [, $err] = BbcWebMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new BbcWebSpec(['step' => 's']),
            'response' => new BbcWebResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new BbcWebResult([]),
        ]);
        [, $err] = BbcWebMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new BbcWebSpec(['step' => 's']),
            'response' => new BbcWebResponse(self::resp(200, ['v' => 1])),
            'result' => new BbcWebResult([]),
        ]);
        [, $err] = BbcWebMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new BbcWebSpec(['step' => 's']),
            'response' => new BbcWebResponse(self::resp(200, ['v' => 2])),
            'result' => new BbcWebResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        BbcWebMakeResponse::call($ctx);
    ...
    {
        $preset = new BbcWebResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new BbcWebSpec([]),
            'response' => new BbcWebResponse([]),
            'result' => new BbcWebResult([]),
        ...
        [$got, $err] = BbcWebMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new BbcWebResult([])]);
        [, $err] = BbcWebMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new BbcWebSpec([]), 'result' => null]);
        [, $err] = BbcWebMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new BbcWebSpec(['step' => 's']),
            'result' => new BbcWebResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new BbcWebOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = BbcWebMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new BbcWebSpec(['step' => 's']),
            'result' => new BbcWebResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new BbcWebOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = BbcWebMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new BbcWebResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new BbcWebSpec([]),
            'result' => new BbcWebResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = BbcWebMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = BbcWebMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new BbcWebError('boom', 'boom');
        $utility->fetcher = function (BbcWebContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new BbcWebSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = BbcWebMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (BbcWebContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = BbcWebMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(BbcWebResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (BbcWebContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new BbcWebSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = BbcWebMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new BbcWebResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new BbcWebSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = BbcWebMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = BbcWebMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = BbcWebMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new BbcWebResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, BbcWebDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new BbcWebResult(['ok' => false])]);
        $this->expectException(BbcWebError::class);
        BbcWebDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new BbcWebResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', BbcWebMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new BbcWebBaseFeature();
        $b = new BbcWebBaseFeature();
        BbcWebFeatureAdd::call($ctx, $a);
        BbcWebFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): BbcWebBaseFeature
    {
        $f = new BbcWebBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        BbcWebFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): BbcWebContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new BbcWebSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = BbcWebPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new BbcWebResponse(['headers' => null]),
            'result' => new BbcWebResult([]),
        ]);
        BbcWebResultHeaders::call($ctx);
