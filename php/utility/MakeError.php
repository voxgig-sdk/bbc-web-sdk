...
// BbcWeb SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class BbcWebMakeError
{
    public static function call(?BbcWebContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new BbcWebContext([], null);
        }
        $op = $ctx->op ?? new BbcWebOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new BbcWebResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof BbcWebError) ? $err->msg : (string)$err;
        $msg = "BbcWebSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new BbcWebError('', $msg, $ctx);
