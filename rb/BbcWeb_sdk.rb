...
class BbcWebSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = BbcWebHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = BbcWebHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, BbcWebFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = BbcWebHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = BbcWebHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = BbcWebHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    allow_op = VoxgigStruct.getpath(@options, "allow.op")
    {
      ...
      "err" => BbcWebError.new(
        "#{op}_allow",
        "BbcWebSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = BbcWebHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = BbcWebError.new(
        "graphql_error", "BbcWebSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
