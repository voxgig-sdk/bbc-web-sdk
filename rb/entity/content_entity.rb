...
class ContentEntity
  ...
  def data_set(args)
    if args
      @_data = BbcWebHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = BbcWebHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Content, Hash] the loaded Content; raises BbcWebError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = BbcWebHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
