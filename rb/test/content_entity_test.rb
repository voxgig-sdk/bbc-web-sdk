...
class ContentEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set BBCWEB_TEST_CONTENT_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def content_basic_setup(extra)
  ...
  entid_env_raw = ENV["BBCWEB_TEST_CONTENT_ENTID"]
  ...
  if env["BBCWEB_TEST_LIVE"] == "TRUE"
    ...
    client = BbcWebSDK.new(Helpers.to_map(merged_opts))
  end
