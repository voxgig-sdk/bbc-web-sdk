...
module BbcWebUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = BbcWebContext.new({}, nil)
    end
    op = ctx.op || BbcWebOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || BbcWebResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(BbcWebError) ? err.msg : err.to_s
    ...
    sdk_err = BbcWebError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(BbcWebError)
