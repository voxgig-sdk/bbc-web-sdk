...
function isRunnable(code: string): boolean {
  return (
    /new\s+BbcWebSDK\b/.test(code) ||
    /\bBbcWebSDK\.test\b/.test(code) ||
    /\bclient\s*\./.test(code)
  )
}
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+BbcWebSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/BbcWebSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
