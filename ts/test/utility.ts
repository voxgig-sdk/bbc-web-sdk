...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.BBC_WEB_TEST_LIVE ||
    'TRUE' === process.env.BBC_WEB_TEST_OVERRIDE
  ) {
  ...
  m.BBC_WEB_TEST_EXPLAIN = process.env.BBC_WEB_TEST_EXPLAIN || m.BBC_WEB_TEST_EXPLAIN
